...
def RMSE(predict_rating_list, true_rating_list):
...
def F1_score_Hit_ratio(recommend_list, purchased_list):
    """计算F1值。
    输入：
...
def NDGG_k(recommend_list, purchased_list):
    """计算NDCG值。
    输入：
        recommend_list： 推荐算法给出的推荐结果。
        purchased_list： 用户的真实购买记录。
...
def evalAll(recommend_list, purchased_list):
    ...
    输出：
        F1，NDCG值。
    """
    f1, p, r, hit_ratio = F1_score_Hit_ratio(recommend_list, purchased_list)
    NDCG = NDGG_k(recommend_list, purchased_list)
